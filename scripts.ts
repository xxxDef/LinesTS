...
namespace logic {
    ...
    export const generateColor = () => getNextRnd(constants.Colors);
    ...
    function createField(gameField: LinesGame, source: GameCell): Distances {
        return Array.from(
            { length: constants.Rows }, (_, row) => Array.from(
                { length: constants.Rows }, (_, col) => {
...
class LinesGame {
    ...
    private nextCircles: NextCircle[] = [];
    ...
    increaseScore(removedCount : number) {
        if (removedCount < 5)
    ...
    getCell = (r: number, c: number): GameCell => this.gameField[r * constants.Rows + c];

    private getEmptyPlace() : GameCell | null {
    ...
    private onClickCell = (cell: GameCell): void => {
        if (cell.color !== null)
    ...
    private onRightClickCell = (cell : GameCell) => {
        if (cell.color === null) {
    ...
    private moveSelectedTo(cell: GameCell) {
    ...
    private removingLines(itemsToRemove: GameCell[]) {
    ...
    private showPath(current: number, path: Pos[], color: Color, lastStep: () => void) {
